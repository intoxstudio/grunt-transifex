...
var Api = module.exports = function(options) {
  ...
  _.bindAll(this, "resources", "resourceDetails", "languageStats",
                  "prepareRequests", "fetchStrings", "writeLanguageFiles", "languages");
};
...
Api.prototype.resourceDetails = function(resources, callback) {
  ...
  async.map(resources, function(resource, step) {
    ...
    grunt.log.ok(details[0].source_language_code);
 ...
 * Unfortunately, a separate call is needed to get the available
 * language codes of each resource.  See below */
Api.prototype.languages = function(callback) {
  var self = this;

  request(this.request("project", this.options.project, "languages"),
  function(err, response, body) {
    /* handle errors */
    if (err) return callback(err);
    if (response.statusCode === 401) {
      return credentials.delete(function() {
        callback(new Error("Invalid Transifex credentials. Aborting."));
      });
    }
    else if (response.statusCode === 404) {
      return callback(new Error("Project slug " + self.options.project + " not found. Aborting."));
    ...
    var retval = _.difference(_.uniq(_.reduce(body,function(acc,val,key,list) {
      return acc.concat(val.translators,val.coordinators,val.reviewers);
    }, [])),self.options.exclude);

    if(self.options.templateFn) {
      retval = self.options.templateFn(retval);
    }
    grunt.config.set("tx_contributors",retval);
